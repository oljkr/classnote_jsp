...
public class BbsDAO { //데이터베이스관련 작업
...
public ArrayList<BbsDTO> list2(String col, String word){
	...
	public int count4() {
		...
		try {
			...
			sql = new StringBuilder();
			sql.append(" SELECT AA.grpno, AA.cnt, BB.subject, BB.indent, AA.cnt ");
			sql.append(" FROM( ");
			sql.append("      SELECT grpno, count(*)-1 as cnt ");
			sql.append("      FROM tb_bbs ");
			sql.append("      GROUP BY grpno ");
			sql.append("      )AA INNER JOIN tb_bbs BB ");
			sql.append(" ON AA.grpno=BB.grpno ");
			sql.append(" WHERE BB.indent=0 ");
			sql.append(" ORDER BY AA.grpno desc ");
